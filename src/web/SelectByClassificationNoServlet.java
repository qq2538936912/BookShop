package web;

import bean.BookType;
import bean.Commodity;
import com.google.gson.Gson;
import dao.BookTypeDao;
import dao.ClassificationDao;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.util.List;

@WebServlet("/SelectByClassificationNoServlet")
public class SelectByClassificationNoServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        int classificationNo = Integer.parseInt(request.getParameter("classificationNo"));
        ClassificationDao classificationDao = new ClassificationDao();
        List<Commodity> commodities = null;
        try {
            commodities = classificationDao.selectbyclassificationNo(classificationNo);
            String gson = new Gson().toJson(commodities);
            response.getWriter().write(gson);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}
